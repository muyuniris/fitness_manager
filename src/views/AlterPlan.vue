   ...
   <div class="content">
    ...
    <el-form :model="form" class="main" size="medium" status-icon label-width="120px" ref="ruleForm" :rules="rules" >
      <el-form-item label="课程名字" prop="course" required>
        ...
        :loading="courseLoading"  remote filterable :remote-method="getCourse" disabled>
      ...
      <el-form-item label="授课教练:" prop="teacher" required ref="teacher">
        <el-select v-model="form.teacher" placeholder="请选择授课教练" @focus="focusFn('teacher')"
      ...
      <el-form-item label="授课场地:" prop="place" required ref="place">
        <el-select v-model="form.place" placeholder="请选择授课场地" @focus="focusFn('place')"
...
<script>
 export default {
    ...
    methods: {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.axios.post('/course/alterPlan',{
      ...
      timeChange(){
        console.log(this.time,this.form.time);
        this.form.teacher='';
      ...
      switchTimeFormat(date) {
        ...
        var arr = [
          `${year}-${this.PrefixInteger(month,2)}-${this.PrefixInteger(day,2)}`,
          time
        ]
        return arr;
      },
      ...
      formateData(data){
        var item={};
        item.id=data.cp_id;
        item.course=data.c_id;
        item.teacher=data.t_id;
        item.place=data.p_id;
        item.max = data.cp_max;
        item.price = data.cp_price;
        item.time=this.switchTimeFormat(data.cp_time);
        console.log(item.time)
        this.teacherList=[{t_id:data.t_id,t_name:data.t_name}]
        this.courseList=[{c_id:data.c_id,c_name:data.c_name}]
        this.placeList=[{p_id:data.p_id,p_name:data.p_name}]
        this.time=item.time;
        return item;
      },
    },
